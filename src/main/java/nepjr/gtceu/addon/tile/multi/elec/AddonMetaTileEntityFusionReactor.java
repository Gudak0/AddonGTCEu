package nepjr.gtceu.addon.tile.multi.elec;
...
import nepjr.gtceu.addon.recipes.ModRecipeMaps;
import nepjr.gtceu.addon.recipes.multiblock.ModFusionEUToStartProperty;
...
public class AddonMetaTileEntityFusionReactor extends RecipeMapMultiblockController implements IFastRenderMetaTileEntity {
    private EnergyContainerList inputEnergyContainers;
    private long heat = 0; // defined in TileEntityFusionReactor but serialized in FusionRecipeLogic
    private Integer color;

    public AddonMetaTileEntityFusionReactor(ResourceLocation metaTileEntityId) {
        super(metaTileEntityId, ModRecipeMaps.HYPER_FUSION_RECIPES);
        this.recipeMapWorkable = new AddonFusionRecipeLogic(this);
    ...
    public MetaTileEntity createMetaTileEntity(IGregTechTileEntity tileEntity) {
        return new AddonMetaTileEntityFusionReactor(this.metaTileEntityId);
    }
    ...
    protected BlockPattern createStructurePattern() {
        return FactoryBlockPattern.start()
                ...
                .where('E', states(getCasingState()).or(metaTileEntities(Arrays.stream(MetaTileEntities.ENERGY_INPUT_HATCH)
                        .filter(mte-> mte != null && mte.getTier() <= GTValues.UV)
                        .toArray(MetaTileEntity[]::new))
                        .setMinGlobalLimited(1).setPreviewCount(16)).or(metaTileEntities(Arrays.stream(AddonMetaTileEntitiesUHV.MOD_ENERGY_INPUT_HATCH)
                        .filter(mte-> mte != null && mte.getTier() <= GTValues.UHV)
                        .toArray(MetaTileEntity[]::new))
    ...
    public List<MultiblockShapeInfo> getMatchingShapes() {
        ...
        MultiblockShapeInfo.Builder baseBuilder = MultiblockShapeInfo.builder()
                ...
                .aisle("######DCD######", "####GG###GG####", "######UCU######")
                .aisle("###############", "######EME######", "###############")
                .where('M', AddonMetaTileEntitiesMultiblock.MOD_FUSION_REACTOR, EnumFacing.SOUTH)
                .where('C', getCasingState())
                .where('G', getCasingState())
                .where('K', getCoilState())
                .where('W', AddonMetaTileEntitiesUHV.MOD_FLUID_EXPORT_HATCH[GTValues.UHV], EnumFacing.NORTH)
                .where('E', AddonMetaTileEntitiesUHV.MOD_FLUID_EXPORT_HATCH[GTValues.UHV], EnumFacing.SOUTH)
                .where('S', AddonMetaTileEntitiesUHV.MOD_FLUID_EXPORT_HATCH[GTValues.UHV], EnumFacing.EAST)
                .where('N', AddonMetaTileEntitiesUHV.MOD_FLUID_EXPORT_HATCH[GTValues.UHV], EnumFacing.WEST)
                .where('w', AddonMetaTileEntitiesUHV.MOD_ENERGY_INPUT_HATCH[GTValues.MAX], EnumFacing.WEST)
                .where('e', AddonMetaTileEntitiesUHV.MOD_ENERGY_INPUT_HATCH[GTValues.MAX], EnumFacing.SOUTH)
                .where('s', AddonMetaTileEntitiesUHV.MOD_ENERGY_INPUT_HATCH[GTValues.MAX], EnumFacing.EAST)
                .where('n', AddonMetaTileEntitiesUHV.MOD_ENERGY_INPUT_HATCH[GTValues.MAX], EnumFacing.NORTH)
                .where('U', AddonMetaTileEntitiesUHV.MOD_FLUID_IMPORT_HATCH[GTValues.UHV], EnumFacing.UP)
                .where('D', AddonMetaTileEntitiesUHV.MOD_FLUID_IMPORT_HATCH[GTValues.UHV], EnumFacing.DOWN)
    ...
    protected void initializeAbilities() {
        ...
        this.energyContainer = new EnergyContainerHandler(this, euCapacity, GTValues.V[14], 0, 0, 0) {
            @Override
            public String getName() {
                return "EnergyContainerInternal";
            }
    ...
    private class AddonFusionRecipeLogic extends MultiblockRecipeLogic {

        public AddonFusionRecipeLogic(AddonMetaTileEntityFusionReactor tileEntity) {
            super(tileEntity);
        }
        ...
        protected long getMaxVoltage() {
            return Math.min(GTValues.V[9], super.getMaxVoltage());
            //return Long.MAX_VALUE;
        }
        ...
        protected boolean checkRecipe(@Nonnull Recipe recipe) {
            ...
            if (recipe.getProperty(ModFusionEUToStartProperty.getInstance(), 0L) > energyContainer.getEnergyCapacity())
                return false;

            long heatDiff = recipe.getProperty(ModFusionEUToStartProperty.getInstance(), 0L) - heat;
            ...
            BloomEffectUtil.requestCustomBloom(RENDER_HANDLER, (buffer)->{
                ...
                if (entity != null) {
                    ...
                    RenderBufferHelper.renderRing(buffer,
                                x + getFrontFacing().getOpposite().getFrontOffsetX() * 7 + 0.5,
                            y + 0.5,
                            z + getFrontFacing().getOpposite().getFrontOffsetZ() * 7 + 0.5,
                            6, 0.2, 10, 20,
